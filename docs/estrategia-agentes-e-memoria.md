...
> **EncerraDigital BNB** — Documento de referência sobre como o projeto é desenvolvido  
...
- Visão geral do projeto (EncerraDigital, contexto BNB)
...
└── adr/
    ...
    ├── ADR-005-*.md            ← Decisão: paleta de cores BNB
...
| ADR-005 | Paleta de cores oficial BNB (vermelho #A6193C, não azul) |
...
│
├── data/
│   ├── agencias.csv                ← 300 agências reais BNB (fonte: BCB/ODbL)
...
│       ├── app.module.ts
│       ├── modules/
│       │   ├── catalogos/          ← /agencias, /agencias/ufs (CSV BNB)
...
│           ├── pdf/                ← gerarTermoEncerramento (14 seções 3303-40-64)
...
│       │   ├── (main)/             ← Route Group: header BNB + max-w-4xl
...
|---|---|---|
| **Fase 1** | Catálogos BNB — 300 agências CSV, 7 motivos, endpoints REST | `eb21b9d` |
...
| **Conformidade** | Paleta BNB verm/laranja, Termo 3303-40-64, campos normativos | — |
...
### Paleta de cores (Manual de Identidade Visual BNB)
| Token Tailwind | HEX | Uso |
|---|---|---|
| `bnb-vermelho` | `#A6193C` | Header, botões primários, destaques |
| `bnb-vermelho-escuro` | `#7A1228` | Hover de botões primários |
| `bnb-laranja` | `#F68B1F` | Badges, ícones, links ativos |
| `bnb-laranja-escuro` | `#C96D0A` | Hover de elementos laranja |
| `bnb-amarelo` | `#FFCB05` | Avisos, status PENDENTE |
| `bnb-salmao` | `#FFE6CB` | Fundos leves, cards de info |
| `bnb-cinza` | `#646464` | Textos secundários |
| `bnb-cinza-claro` | `#F5F5F5` | Fundos de seções |
| `bnb-verde` | `#07A684` | Status CONCLUÍDO, sucesso |
...
_Documento gerado pelo Agente Documentador — EncerraDigital BNB | 2026-04-06_  
