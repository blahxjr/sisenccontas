...
import {
  encerramentoSchema,
  type EnccerramentoFormData,
} from "@lib/encerramento-schema";
...
export function FormularioEncerramento() {
  ...
  const { agencias, carregando: carregandoAgencias } =
    useAgencias(ufSelecionada);

  useEffect(() => {
    setValue("agencia", "");
  }, [ufSelecionada, setValue]);

  const camposEtapa1: (keyof EnccerramentoFormData)[] = [
    "uf",
    "agencia",
    "numeroConta",
    "titularNome",
  ];
  const camposEtapa2: (keyof EnccerramentoFormData)[] = ["enderecoCliente"];
  ...
  const onSubmit = async (dados: EnccerramentoFormData) => {
    setEnviando(true);
    setErroEnvio(null);
    try {
      const payload = {
        ...
        numeroChequeDevolvido: dados.possuiCheque
          ? dados.numeroChequeDevolvido
          : undefined,
        possuiSaldoPositivo: dados.possuiSaldoPositivo,
        bancoTransferencia: dados.possuiSaldoPositivo
          ? dados.bancoTransferencia
          : undefined,
        agenciaTransferencia: dados.possuiSaldoPositivo
          ? dados.agenciaTransferencia
          : undefined,
        contaTransferencia: dados.possuiSaldoPositivo
          ? dados.contaTransferencia
          : undefined,
      };
          ...
          return (
            <li key={idx} className="flex-1 flex items-center">
              <div className="flex flex-col items-center flex-1">
                ...
                <span
                  className={`text-xs mt-1 text-center hidden sm:block ${ativa ? "text-brf-vermelho font-semibold" : "text-gray-500"}`}
                >
                  {nome}
                </span>
              </div>
              {idx < ETAPAS.length - 1 && (
                <div
                  className={`h-0.5 flex-1 mx-1 ${idx < etapaAtual ? "bg-brf-laranja" : "bg-gray-200"}`}
                />
              )}
            </li>
          );
      ...
      {etapaAtual === 1 && (
        <div className="space-y-5">
          <p className="text-sm text-gray-600 bg-brf-salmao border border-brf-laranja/30 rounded-lg p-3">
            Conforme normativo BRF-3303-03-11, estas informações são necessárias
            para o processamento do encerramento.
          </p>
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm text-gray-700 font-medium">
                Movimentei a conta através de cheque
              </span>
            </label>
            {possuiCheque && (
              <div className="ml-7">
          ...
          <div className="space-y-2">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <span className="text-sm text-gray-700 font-medium">
                A conta possui saldo positivo
              </span>
            </label>
            {possuiSaldoPositivo && (
              <div className="ml-7 space-y-3">
                <p className="text-xs text-gray-500">
                  Informe a conta de destino para transferência do saldo:
                </p>
                <div>
      ...
      {etapaAtual === 2 && (
        <div className="space-y-5">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            <label className="flex items-start gap-3 cursor-pointer">
              ...
              <span className="text-sm text-gray-700 leading-relaxed">
                ...
                <strong>Termo de Encerramento de Conta Corrente</strong>,
                conforme normativo BRF-3303-03-11. Declaro que as informações
                são verdadeiras e que sou o titular da conta informada. Concordo
                com a{" "}
                <a
                  href="#"
                  className="text-brf-laranja underline hover:text-brf-vermelho"
                >
          ...
          <p className="text-xs text-gray-400 text-center">
            🔒 Seus dados são protegidos por criptografia AES-256. Apenas
            operadores autorizados do BRF têm acesso.
          </p>
        </div>
      )}
    </form>
